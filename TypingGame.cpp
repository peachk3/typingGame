...
std::vector<std::vector<std::wstring>> TypingGame::loadSentencesFromFile(const std::string& filePath) {
    std::vector<std::vector<std::wstring>> sentences;
    std::wcout << L"[디버깅] 파일 로드 시작: " << std::endl;

    std::wifstream file(filePath);
    file.imbue(std::locale("")); // 유니코드 지원

    if (!file) {
        std::wcout << L"파일을 열 수 없습니다: " << std::endl; //
        return sentences;
    }
    ...
    while (std::getline(file, line)) {
        if (!line.empty()) {
            sentences.push_back({ line }); // 한 줄을 하나의 문장 벡터로 저장
        }
    }

    std::cout << "[디버깅] 총 문장 수: " << sentences.size() << std::endl;
    return sentences;
}


// 게임 시작 함수
void TypingGame::startNewGame(sf::RenderWindow& gameWindow, sf::Font& font, int fontSize, std::vector<std::vector<std::wstring>>& sentences)
{
    std::cout << "[디버깅] startNewGame 실행됨!" << std::endl;
    ...
    while (gameWindow.isOpen()) {
                    ...
                    if (currentInput == sentences[currentSentenceIdx][currentLineIdx])
                    {
                        ...
                        if (currentLineIdx >= sentences[currentSentenceIdx].size())
                        {
                            ...
                            if (currentSentenceIdx >= sentences.size())
                            {
                                ...
                                // 전체 문장 종료 결과창 출력
                                showResultWindow(gameWindow, font, fontSize, totalWpm / totalLines,
        ...
        drawOriginalText(gameWindow, font, fontSize, sentences);
        drawUserInputText(gameWindow, font, fontSize, userInputs, sentences);
    ...
    std::vector<std::vector<std::wstring>>& sentences)
{
    ...
    //bool goBackToMain = false; // 메인으로 돌아가는지 여부
    ...
    //if (goBackToMain) return; // 게임을 다시 시작하지 않고 메인으로 돌아감
}
    ...
    std::vector<std::vector<std::wstring>>& fileAllLines)
{
    ...
    for (int i = 0; i < fileAllLines.size(); ++i)
    {
        ...
        {
            // 기준 문장 출력
            sf::Text originalText(font, fileAllLines[i][j], fontSize);
            originalText.setFillColor(sf::Color::Black);
            originalText.setPosition({ x, y });
            window.draw(originalText);


            y += originalText.getLocalBounds().size.y + fontSize * 3; // 줄 간격 설정
